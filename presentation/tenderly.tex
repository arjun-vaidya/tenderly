...
\begin{frame}{DigitalOcean Platform Usage}
  ...
  \begin{tikzpicture}[
    ...
    % Left side: User -> Frontend -> Backend (horizontal)
    \node[font=\small\bfseries, text=tenderlyInk] (user) {User};
    \node[svc, right=1cm of user] (fe) {Static Site\\{\footnotesize React + Vite}};
    \node[svc, right=of fe] (api) {Web Service\\{\footnotesize FastAPI}};

    % Right side: three services stacked vertically
    \node[ext, right=of api, yshift=1.2cm] (ai) {DO AI Platform\\{\footnotesize Gradient}};
    \node[ext, right=of api] (db) {DO Managed\\{\footnotesize PostgreSQL}};
    \node[ext, right=of api, yshift=-1.2cm] (soc) {SF 311 Socrata\\{\footnotesize Open Data}};

    % Arrows: left chain
    \draw[arr] (user) -- node[lbl, above] {HTTPS} (fe);
    \draw[arr] (fe) -- node[lbl, above] {REST API} (api);

    % Arrows: right side with clean right-angle paths
    \draw[arr] (api.east) ++(0,0.15) -- ++(0.35,0) |- (ai.west)
      node[lbl, pos=0.75, above] {profile + explanations};
    \draw[arr] (api.east) -- (db.west)
      node[lbl, pos=0.5, above] {read/write};
    \draw[arr] (api.east) ++(0,-0.15) -- ++(0.35,0) |- (soc.west)
      node[lbl, pos=0.75, below] {7-day needs data};
