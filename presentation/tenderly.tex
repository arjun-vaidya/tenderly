...
\begin{frame}{DigitalOcean Platform Usage}
  ...
  \begin{tikzpicture}[
    ...
    % Right side: three services stacked vertically with generous spacing
    \node[ext, right=2.8cm of api, yshift=1.3cm] (ai) {DO AI Platform\\{\footnotesize Gradient}};
    \node[ext, right=2.8cm of api] (db) {DO Managed\\{\footnotesize PostgreSQL}};
    \node[ext, right=2.8cm of api, yshift=-1.3cm] (soc) {SF 311 Socrata\\{\footnotesize Open Data}};
    ...
    \draw[arr] (api.east) ++(0,0.12) -- ++(0.7,0) |- (ai.west);
    \node[lbl] at ($(api.east)+(1.4, 0.75)$) {profile + explanations};

    % Arrow to DB: straight horizontal, label centered
    \draw[arr] (api.east) -- (db.west);
    \node[lbl, above] at ($(api.east)!0.5!(db.west)$) {read / write};

    % Arrow to Socrata: right-angle, label on the horizontal segment
    \draw[arr] (api.east) ++(0,-0.12) -- ++(0.7,0) |- (soc.west);
    \node[lbl] at ($(api.east)+(1.4, -0.75)$) {7-day needs data};

    % Dashed boundary around DO-hosted services
    \node[draw=tenderlyForest!40, dashed, rounded corners=8pt, line width=0.6pt,
          fit=(fe)(api)(ai)(db)(soc), inner sep=12pt,
